...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <p class="text-sm text-slate-500">Cập nhật điểm đến</p>
    ...
    <form method="POST" action="{{ route('admin.destinations.update', $destination) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <x-admin.card title="Thông tin cơ bản">
            <div class="grid gap-4 md:grid-cols-2">
                <x-admin.input label="Tên điểm đến" name="name" value="{{ old('name', $destination->name) }}" required />
                <x-admin.input label="Slug" name="slug" value="{{ old('slug', $destination->slug) }}" />
                <x-admin.input label="Tỉnh/Thành phố" name="province" value="{{ old('province', $destination->province) }}" />
                ...
                <div class="space-y-1">
                    ...
                    <select name="region" class="input">
                        ...
                        @foreach(['Miền Bắc', 'Miền Trung', 'Miền Nam'] as $region)
                            <option value="{{ $region }}" @selected(old('region', $destination->region) === $region)>{{ $region }}</option>
                        @endforeach
                    </select>
                    @error('region')
                ...
                <div class="space-y-2">
                    ...
                    @if($destination->thumbnail_path)
                        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
                            <img src="{{ $destination->thumbnail_url }}" class="h-40 w-full object-cover" alt="{{ $destination->name }}">
                        </div>
                    @endif
                    @error('thumbnail')
                        <p class="text-xs text-red-500">{{ $message }}</p>
        ...
        <x-admin.card title="Mô tả">
            <div class="space-y-1">
                ...
                <textarea name="description" rows="5" class="input">{{ old('description', $destination->description) }}</textarea>
                @error('description')
                    <p class="text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex flex-wrap gap-3">
            <button type="submit" class="btn-primary">Cập nhật điểm đến</button>
